...
import type { OnlineManga } from "@/lib/data";
import {
  Card,
  CardContent,
  CardHeader,
...
export function OnlineMangaCard({ manga }: OnlineMangaCardProps) {
  ...
  const isInLibrary = isMangaInLibrary({
      ...
      <CardContent className="p-3 flex-grow flex flex-col">
        ...
        <div className="mt-auto pt-2 space-y-2">
          <div className="text-sm text-muted-foreground truncate">
            {manga.score && <span>Nota: {manga.score.toFixed(2)}</span>}
            {manga.score && (manga.totalChapters || manga.status) ? " • " : ""}
            {manga.totalChapters > 0 ? <span>{manga.totalChapters} caps</span> : manga.status && <span>{manga.status}</span>}
          </div>
          <Button variant={isInLibrary ? "secondary" : "default"} className="w-full" onClick={handleAdd} disabled={isInLibrary}>
